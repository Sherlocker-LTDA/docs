...
- **Borderô** (`POST /v1/operations`): a cobrança acontece normalmente na criação, mas **não aparece no payload** — a resposta 202 traz apenas `operation_id`, `status` e `created_at`. Acompanhe débitos e estornos no extrato de tokens do workspace, no app Sherlocker.
