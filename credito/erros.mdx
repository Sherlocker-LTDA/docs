...
| [`forbidden`](/problems/forbidden) | 403 | `engine_id` de uma engine de outro workspace | — |
| [`not_found`](/problems/not_found) | 404 | Operação inexistente ou de outro workspace; `engine_id` inexistente | — |
| [`idempotency_conflict`](/problems/idempotency_conflict) | 409 | Mesma `Idempotency-Key` com requisição ainda em andamento, ou mesma key com conteúdo diferente (o hash cobre o SHA-256 dos arquivos + `engine_id`) | — |
| [`no_engine`](/problems/no_engine) | 422 | Nenhuma engine de borderô disponível para a operação | — |
...
async function handleSherlockerError(res) {
  ...
  switch (problem.code) {
    ...
    case "forbidden":
      throw new Error("Engine de outro workspace ou acesso negado");
