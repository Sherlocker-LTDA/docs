...
| `failed` | Sim | O motor não conseguiu concluir a operação; a resposta não traz `summary` nem `titulos` e os tokens são estornados |
...
| `incompleto` | Alguma verificação do título ficou indisponível (fonte de dados falhou), sem nenhum bloqueio — mesma semântica do `incompleto` do Motor de Análise |

O `summary` agrega as contagens em `aprovados`, `alertas` e `reprovados`, junto com `valor_total` e `valor_aprovado` para a decisão financeira da operação.

## Verdict do cedente

O campo `summary.cedente_verdict` carrega o veredito da análise de risco do **cedente** — a mesma semântica de `verdict` do [Motor de Análise](/motor-analise/ciclo-de-vida) (`aprovado` | `alerta` | `reprovado` | `incompleto`), ou `null` se o veredito do cedente não estiver disponível (o campo é preenchido em melhor esforço). Como o cedente é a contraparte de todos os títulos, um `cedente_verdict` de `reprovado` compromete o borderô inteiro, independentemente dos sacados.
...
- `cedente_analysis_id` e `sacado_analysis_id` podem ser `null` quando a análise da entidade não está disponível — os campos são preenchidos em melhor esforço.
...
É assim que os dois produtos compõem: o Motor de Borderô responde "o que fazer com cada título"; o drill-down nas análises responde "por quê".
...
Os campos de resultado chegam de uma vez quando a operação atinge `completed`: `finished_at`, `summary` e `titulos[]`. Não há resultado parcial: ou a operação está em andamento, ou está pronta.

Em `failed`, a resposta ganha apenas `finished_at` — sem `summary` nem `titulos` — e os tokens debitados na criação são estornados automaticamente.
...
if data["status"] == "completed":
    ...
    # failed: sem summary/titulos; tokens estornados automaticamente
...
- Consultar uma operação inexistente ou de outro workspace retorna `404 not_found` — veja [Erros](/motor-analise/erros).
