...
A base URL dos exemplos é `https://221b-api.sherlocker.com.br/api/v1`. A autenticação é feita pelo header `Authorization: Bearer slhk_sua_chave_aqui`; os Motores não aceitam `?token=` na URL.
...
- Operações que terminam em `failed` são estornadas automaticamente.
