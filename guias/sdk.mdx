...
<img src="/images/sketches/integration-sdk.svg" alt="O SDK organiza a passagem entre aplicação e API." />
...
| `buscarListasRestritivas()` | Sanções, PEP e restrições (11 listas) | 3 |
...
async function backgroundCheck(cpf: string) {
  ...
  const [info, empresas, processos, dividas, listas] = await Promise.all([
    pessoa.buscarInfo(),
    pessoa.buscarEmpresasSocio(),
    pessoa.buscarProcessos(),
    pessoa.buscarDividas(),
    pessoa.buscarListasRestritivas(),
  ]);

  return { info, empresas, processos, dividas, listas };
}
```
...
async function dueDiligence(cnpj: string) {
  ...
  const [info, socios, processos, dividas, listas, licitacoes] =
    await Promise.all([
      empresa.buscarInfo(),
      empresa.buscarSocios(),
      empresa.buscarProcessos(),
      empresa.buscarDividas(),
      empresa.buscarListasRestritivas(),
      empresa.buscarLicitacoes(),
    ]);

  return { info, socios, processos, dividas, listas, licitacoes };
}
```
