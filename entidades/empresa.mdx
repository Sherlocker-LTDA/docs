...
<Note>
  Os dados cadastrais são atualizados mensalmente, conforme a base da Receita
  Federal.

  - **Validar o nome D+0:** `GET /empresas/cnpj/{cnpj}?realtime=true`
  - **Confirmar a situação e gerar o comprovante:** `POST /certidoes/cnpj`

  Consultas D+0 são assíncronas e podem levar de segundos a alguns minutos.
  [Saiba quando usar cada opção](/cadastro/performance-acuracia).
</Note>
...
| `porte` | string \| null | `Não Informado`, `Microempresa`, `Empresa de Pequeno Porte`, `Média ou Grande Porte`. Códigos não mapeados retornam o valor bruto da Receita |
...
|-----------|------|--------|-----------|
| `limit` | integer | 50 | Máximo de registros retornados (max 200) |
