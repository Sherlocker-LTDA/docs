...
<Note>
  Esta consulta prioriza performance e cobertura usando a base consolidada da
  Sherlocker. Se você precisa confirmar o nome associado ao documento em uma
  fonte oficial no momento da decisão, acrescente `?realtime=true` a
  `GET /pessoas/cpf/{cpf}`. Os dados da base são devolvidos imediatamente e a
  resposta inclui uma validação assíncrona. Ela normalmente leva dezenas de
  segundos e, conforme a disponibilidade do portal oficial, pode levar alguns
  minutos. Para confirmar também a situação cadastral do CPF na Receita, use
  `POST /certidoes/cpf`, enviando o CPF no corpo da requisição. Veja
  [Consultas D+0](/cadastro/performance-acuracia).
</Note>
...
<CardGroup cols={2}>
  <Card title="Nome rápido" icon="id-card" href="/api-reference/pessoas/nome-e-genero">
    Apenas nome, nascimento e sexo
  </Card>
  <Card title="Endereços" icon="location-dot" href="/api-reference/pessoas/enderecos">
    Histórico de endereços
  </Card>
  <Card title="Telefones" icon="phone" href="/api-reference/pessoas/telefones">
    Telefones fixos e móveis
  </Card>
  <Card title="Emails" icon="envelope" href="/api-reference/pessoas/emails">
    Emails com score de confiabilidade
  </Card>
  <Card title="Parentes" icon="people-arrows" href="/api-reference/pessoas/parentes">
...
<CardGroup cols={2}>
  ...
  <Card title="Empregos" icon="briefcase" href="/api-reference/busca-reversa/historico-profissional">
