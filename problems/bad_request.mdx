...
- Header `Idempotency-Key` ausente ou vazio no `POST /v1/analyses` (detail: `Idempotency-Key header required`). No `POST /v1/operations` (Motor de Borderô), a key é **opcional** — a ausência não é erro.
...
- Envie o header `Idempotency-Key` em todo `POST /v1/analyses` (e, embora opcional, também no `POST /v1/operations` quando houver retry). Recomendamos um UUID, amarrado ao ID interno do job (ou do borderô) no seu sistema.
