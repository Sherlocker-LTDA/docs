...
- **Motor de Borderô** (`POST /v1/operations`, `multipart/form-data`): os `errors[]` são field-level sobre as partes do multipart — `file`, `xmls` e `cedenteCnpj`. Exemplos de mensagens: `No file provided` (parte `file` ausente), `cedenteCnpj is required`, `Invalid CNPJ format` (dígito verificador), arquivo que não parseia como CNAB 400 remessa, ou arquivo acima do teto (16 MB para `file`; 32 MB para o ZIP `xmls`). Nada é cobrado — o débito só acontece após o parse bem-sucedido do CNAB.
- **Motor de Borderô** (`GET /v1/operations/{id}/result`): parâmetro `filter` com valor fora do enum (detail: `Invalid filter value. Use "issues" or "all".`).
