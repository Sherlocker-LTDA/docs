...
- **`POST /v1/operations`**: o payload multipart — o conteúdo dos arquivos (`file` e `xmls`) e os campos (`cedente_cnpj`, `engine_id`, `purpose`, `legal_basis`). Trocar o arquivo, o ZIP ou qualquer campo com a mesma key gera conflito.
...
<Note>
  No `POST /v1/operations` (Motor de Borderô), o header `Idempotency-Key` é **opcional**: este erro só pode ocorrer quando você envia a key. Sem ela, cada POST cria uma operação nova — sem deduplicação e sem conflito. No `POST /v1/analyses`, a key continua obrigatória.
</Note>
