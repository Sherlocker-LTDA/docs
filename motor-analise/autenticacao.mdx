---
title: "Autenticação"
description: "Como autenticar na API do Motor de Análise com chave Bearer"
icon: "key"
---

Toda requisição à API `/v1` do Motor de Análise exige uma chave de API no header `Authorization`, no formato Bearer:

```
Authorization: Bearer slhk_sua_chave_aqui
```

As chaves têm o prefixo `slhk_` e são vinculadas ao seu workspace no Sherlocker.

<Warning>
  **Diferente do restante das docs deste site.** As demais APIs documentadas aqui (221b-api) autenticam via query param `?token=`. A API do Motor de Análise **não aceita** token em query param — apenas o header `Authorization: Bearer`. O host também é outro: não use o host das outras docs nos exemplos abaixo.
</Warning>
...
Um `GET` em um run inexistente serve como teste rápido da chave — não debita tokens. Resposta `404 not_found` significa que a autenticação funcionou; `401` indica problema na chave; `403 feature_not_enabled` indica chave válida, mas workspace sem a feature `analysis_engine` — veja a tabela abaixo.

<CodeGroup>
```bash cURL
curl -i "https://<API_HOST>/v1/analyses/00000000-0000-0000-0000-000000000000" \
...
resp = requests.get(
    "https://<API_HOST>/v1/analyses/00000000-0000-0000-0000-000000000000",
...
O acesso à API é liberado por workspace: a feature `analysis_engine` cobre qualquer chamada `/v1`, e análises de CPF exigem adicionalmente a feature `analysis_engine_pf`.
...
<CodeGroup>
```bash cURL
curl "https://<API_HOST>/v1/analyses/00000000-0000-0000-0000-000000000000" \
  -H "Authorization: Bearer slhk_sua_chave_aqui" \
