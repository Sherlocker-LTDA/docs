...
<Warning>
  **Diferente do restante das docs deste site.** As demais APIs documentadas aqui (221b-api) autenticam via query param `?token=`. A API `/v1` dos Motores de Análise **não aceita** token em query param — apenas o header `Authorization: Bearer`. O host também é outro: não use o host das outras docs nos exemplos abaixo.
</Warning>
...
O acesso à API é liberado por workspace: a feature `analysis_engine` cobre qualquer chamada `/v1` — os dois motores, incluindo `POST /v1/operations` — e análises de CPF exigem adicionalmente a feature `analysis_engine_pf`.
