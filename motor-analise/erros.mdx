...
|---|---|---|---|
| [`validation_error`](/problems/validation_error) | 400 | **CPF/CNPJ:** campo do body com tipo/enum inválido. **Borderô:** parte `file` ausente, `cedenteCnpj` ausente ou inválido, CNAB que não parseia, arquivo acima do teto; `filter` inválido no `GET /result` | `errors[]` |
...
| [`conflict`](/problems/conflict) | 409 | **Borderô:** `GET /v1/operations/{id}/result` antes de a operação chegar a `completed` | — |
...
No Motor de Borderô, os mesmos `errors[]` reportam problemas field-level com as partes do multipart — `file`, `xmls` e `cedenteCnpj` (ex.: `{ "field": "file", "message": "No file provided" }`, `{ "field": "cedenteCnpj", "message": "Invalid CNPJ format" }`). Nesse caso nada é cobrado — no borderô, o débito só acontece após o parse bem-sucedido do CNAB.
...
def handle_sherlocker_error(resp: requests.Response) -> None:
    ...
    if code == "conflict":
        # Bordero: /result antes de completed — continue o polling do GET /v1/operations/{id}
        raise SherlockerApiError(problem)
