---
title: "Erros"
description: "Índice de erros da API do Motor de Análise: formato RFC 7807, códigos e tratamento"
icon: "triangle-exclamation"
---

Toda resposta de erro da API `/v1` segue o [RFC 7807](https://datatracker.ietf.org/doc/html/rfc7807) e é retornada com `Content-Type: application/problem+json`. Isso vale para os dois endpoints (`POST /v1/analyses` e `GET /v1/analyses/{id}`).
...
```bash
curl -i "https://<API_HOST>/v1/analyses/a1b2c3d4-0000-0000-0000-000000000001"
```
...
Cada código tem uma página própria em `/problems/<code>`:

| `code` | HTTP | Quando acontece | Campos extras |
|---|---|---|---|
| [`validation_error`](/problems/validation_error) | 400 | Campo do body falhou validação (tipo/enum) | `errors[]` |
| [`bad_request`](/problems/bad_request) | 400 | `Idempotency-Key` ausente; `document` sem dígitos, com tamanho errado ou dígito verificador inválido | — |
| [`unauthorized`](/problems/unauthorized) | 401 | Header `Authorization` ausente/malformado; chave de API inválida ou revogada | — |
| [`insufficient_tokens`](/problems/insufficient_tokens) | 402 | Saldo de tokens do workspace insuficiente | `required`, `balance` |
| [`feature_not_enabled`](/problems/feature_not_enabled) | 403 | Feature `analysis_engine` (qualquer chamada `/v1`) ou `analysis_engine_pf` (análises de CPF) desabilitada no workspace | `request_access_url` |
| [`forbidden`](/problems/forbidden) | 403 | 403 genérico, não relacionado a feature | — |
| [`not_found`](/problems/not_found) | 404 | Run inexistente ou pertencente a outro workspace | — |
| [`idempotency_conflict`](/problems/idempotency_conflict) | 409 | Mesma `Idempotency-Key` com requisição ainda em andamento, ou mesma key com body diferente | — |
| [`no_engine`](/problems/no_engine) | 422 | Sem engine configurada no workspace; `engine_id` inválido/arquivado; `subject_type` incompatível com a engine | — |
...
Informa quantos tokens a análise exigia (`required`) e o saldo atual do workspace (`balance`). Os valores abaixo são **ilustrativos** — o custo por análise está em definição durante o beta:
...
O saldo é gerenciado no app Sherlocker — veja [Cobrança](/motor-analise/cobranca).
...
- `validation_error` / `bad_request`: corrija a requisição. Se for reenviar um POST corrigido, use uma `Idempotency-Key` **nova** (a mesma key com body diferente retorna `409`).
- `idempotency_conflict` com requisição em andamento: não reenvie o POST — consulte `GET /v1/analyses/{id}` com o `id` da resposta 202 original.
- `server_error` (e timeouts de rede): reenvie **com a mesma** `Idempotency-Key`, com back-off. Detalhes em [Idempotência](/motor-analise/idempotencia).
...
def handle_sherlocker_error(resp: requests.Response) -> None:
    ...
    if code == "validation_error":
        # Corrija a requisicao: veja problem["errors"]
    ...
    if code == "idempotency_conflict":
        # Consulte o run existente em vez de repetir o POST
        raise SherlockerApiError(problem)
    ...
    # retry com back-off; inclua problem["instance"] ao acionar o suporte
    raise SherlockerApiError(problem)
```
...
async function handleSherlockerError(res) {
  ...
  switch (problem.code) {
    ...
    case "idempotency_conflict":
      // Consulte o run existente em vez de repetir o POST
      throw new Error("Requisicao concorrente em andamento: faca polling do run original");

    case "no_engine":
      throw new Error("Nenhuma engine configurada para este workspace");
