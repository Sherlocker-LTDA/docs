...
Toda resposta de erro da API `/v1` segue o [RFC 7807](https://datatracker.ietf.org/doc/html/rfc7807) e é retornada com `Content-Type: application/problem+json`. Isso vale para os **dois motores** — Motor de CPF/CNPJ (`POST /v1/analyses`, `GET /v1/analyses/{id}`) e Motor de Borderô (`POST /v1/operations`, `GET /v1/operations/{id}`): mesmo formato, mesmos códigos.
...
|---|---|---|---|
| [`validation_error`](/problems/validation_error) | 400 | **CPF/CNPJ:** campo do body com tipo/enum inválido. **Borderô:** `cnab_base64` ausente, base64 inválido, CNAB que não parseia, arquivo acima do limite | `errors[]` |
| [`bad_request`](/problems/bad_request) | 400 | **Ambos:** `Idempotency-Key` ausente. **CPF/CNPJ:** `document` sem dígitos, tamanho errado ou dígito verificador inválido | — |
...
| [`idempotency_conflict`](/problems/idempotency_conflict) | 409 | **Ambos:** mesma `Idempotency-Key` em andamento, ou reutilizada com conteúdo diferente — no CPF/CNPJ o hash cobre o body; no borderô, o SHA-256 dos bytes dos arquivos + `engine_id` + `legal_basis` | — |
| [`no_engine`](/problems/no_engine) | 422 | **Ambos:** engine arquivada ou incompatível; nenhuma engine disponível. **Borderô:** engine sem nenhum bloco ativo | — |
...
No Motor de Borderô, os mesmos `errors[]` reportam problemas com os arquivos (ex.: `{ "field": "cnab_base64", "message": "file is not a valid CNAB 400 remessa" }`). Nesse caso nada é cobrado — no borderô, o débito só acontece após o parse bem-sucedido do CNAB.
...
O saldo é gerenciado no app Sherlocker — veja [Cobrança do Motor de CPF/CNPJ](/motor-analise/cobranca) e [Cobrança do Motor de Borderô](/credito/cobranca).
...
- `idempotency_conflict` com requisição em andamento: não reenvie o POST — consulte via `GET` (`/v1/analyses/{id}` ou `/v1/operations/{id}`) com o `id` da resposta 202 original.
- `server_error` (e timeouts de rede): reenvie **com a mesma** `Idempotency-Key`, com back-off. Detalhes em [Idempotência (CPF/CNPJ)](/motor-analise/idempotencia) e [Idempotência (Borderô)](/credito/idempotencia).
...
def handle_sherlocker_error(resp: requests.Response) -> None:
    """Tratamento unico para /v1/analyses e /v1/operations."""
    problem = resp.json()
