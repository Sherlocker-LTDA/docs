...
Este é o motor de **documento individual**. Se o seu caso é analisar uma operação de desconto de duplicatas a partir de um arquivo CNAB 400 — cedente, sacados e títulos de uma vez —, use o **[Motor de Borderô](/credito/introducao)**: mesma base `/v1`, mesma chave `slhk_`, mesma feature. Os dois compõem: cada título do resultado do borderô referencia análises normais desta API, consultáveis via `GET /v1/analyses/{id}`. A comparação completa está em [Motores de Análise](/motores/introducao).
