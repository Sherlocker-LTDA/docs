...
Não confunda com o **[Motor de Crédito](/credito/introducao)**, também documentado neste site. São produtos complementares que compartilham a mesma base `/v1`, a mesma chave `slhk_` e a mesma feature `analysis_engine`:
...
Se o seu caso é analisar uma operação de desconto de duplicatas a partir de um arquivo CNAB 400, use o Motor de Crédito. Se é avaliar o risco de um CNPJ ou CPF específico, esta é a API certa. Os dois compõem: cada título do resultado do Motor de Crédito referencia análises normais desta API, consultáveis via `GET /v1/analyses/{id}`.
